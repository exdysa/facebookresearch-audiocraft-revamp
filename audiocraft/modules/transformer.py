...
class StreamingMultiheadAttention(StreamingModule):
    ...
    def __init__(
        ...
        if self.custom:
            ...
            assert kv_repeat == 1
            self.mha = nn.MultiheadAttention(embed_dim, num_heads, dropout=dropout, bias=bias, batch_first=True, **factory_kwargs)
    ...
    def _get_mask(self, current_steps: int, device: torch.device, dtype: torch.dtype):
        ...
        return torch.where(valid, torch.zeros([], device=device, dtype=dtype), torch.full([], float("-inf"), device=device, dtype=dtype))
    ...
    def forward(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, key_padding_mask=None, need_weights=False, attn_mask=None, average_attn_weights=True, is_causal=False):
        assert not is_causal, "New param added in torch 2.0.1 not supported, use the causal args in the constructor."
        ...
        dtype = query.dtype
        if self._is_streaming:
            assert self.causal or self.cross_attention, "Streaming only available for causal or cross attention"
        ...
        if self.custom:
            ...
            if self.memory_efficient:
                ...
                if _efficient_attention_backend == "torch":
                    x = torch.nn.functional.scaled_dot_product_attention(q, k, v, is_causal=attn_mask is not None, dropout_p=p)
                    try:
                        from xformers import ops
            ...
            x, _ = self.mha(query, key, value, key_padding_mask, need_weights, attn_mask, average_attn_weights)
            x = x.to(dtype)
...
class StreamingTransformerLayer(nn.TransformerEncoderLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(d_model, num_heads, dim_feedforward, dropout, device=device, dtype=dtype, batch_first=True, **kwargs)
        factory_kwargs = {"device": device, "dtype": dtype}
        ...
        self.self_attn: StreamingMultiheadAttention = StreamingMultiheadAttention(causal=causal, past_context=past_context, rope=rope, qk_layer_norm=qk_layer_norm, kv_repeat=kv_repeat, **attn_kwargs, **factory_kwargs)  # type: ignore
        ...
        if cross_attention:
            self.cross_attention = StreamingMultiheadAttention(cross_attention=True, qk_layer_norm=qk_layer_norm_cross, **attn_kwargs, **factory_kwargs)
            # Norm and dropout
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.positional_embedding in ["rope", "sin_rope"]:
            ...
            self.rope = RotaryEmbedding(d_model // num_heads, max_period=max_period, xpos=xpos, scale=positional_scale, device=device)
